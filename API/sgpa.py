...
def calc(sub, subj, count4, count2, count3, count1, record):
    c = count4 * 4 + count3 * 3 + count2 * 2 + count1 * 1
    cp = 0
...
def gpa(college, year, branch, low, high, sem):
        ...
        for i in range(0, sheet.nrows):
            ...
            for j in range(5, marks_code, 5):
                if sheet.cell_value(i, j + 1) == 'P':
                    ...
                    subj.append(sheet.cell_value(i, j - 3))
                    sub.append(0)
            record, sgpa = calc(sub, subj, count4, count2, count3, count1, record)
